...
const EMGSerial = () => {
  ...
  const processSerialData = (data) => {
    ...
    if (!isNaN(numericValue)) {
      setStats((prev) => {
        const currentMin =
          prev.min === 0 ? numericValue : Math.min(prev.min, numericValue);
        const currentMax = Math.max(prev.max, numericValue);
    ...
    if (rawMatch) {
      const rawNumber = parseFloat(rawMatch[1]);

      if (!isNaN(rawNumber)) {
        setStats((prev) => {
          const currentMin =
            prev.min === 0 ? rawNumber : Math.min(prev.min, rawNumber);
          const currentMax = Math.max(prev.max, rawNumber);
  ...
  const predictFromEMG = (rawValue, magValue, thrValue) => {
    ...
    if (demoMode === "paralysis") {
      activity = "Low Signal";
      gesture = "Weak / No Movement";
      confidence = 92;
  ...
  const generateRecommendations = (activity, fatigue, mag, raw) => {
    ...
    if (demoMode === "paralysis") {
      suggestions.push({
        ...
        icon: "🧠",
        priority: "high",
        title: "Paralysis Demo Feed Running",
        message:
          "Very low EMG activity detected. This pattern represents weak or reduced muscle response.",
        action: "Physiotherapy / Assisted Movement",
      });

      suggestions.push({
        id: "demo-paralysis-care",
        icon: "🦾",
        priority: "medium",
        title: "Suggested Measure",
        message:
          "Monitor muscle response continuously and perform guided rehabilitation exercises.",
        action: "Start Rehab Monitoring",
      });
    ...
    if (demoMode === "normal") {
      suggestions.push({
        ...
        icon: "✅",
        priority: "info",
        title: "Normal Person Demo Feed Running",
        message:
          "EMG signal is showing normal muscle response and stable activity pattern.",
    ...
    if (fatigue === "High Fatigue") {
      suggestions.push({
        ...
        title: "Immediate Rest Required",
        message:
          "High muscle fatigue detected. Take a 15-20 minute break immediately to prevent injury.",
        action: "Stop Activity",
      });
      ...
      suggestions.push({
        ...
        title: "Gentle Stretching",
        message:
          "Perform light stretching exercises to reduce muscle tension and improve recovery.",
        action: "Start Stretching",
      });
    } else if (fatigue === "Moderate Fatigue") {
      suggestions.push({
        ...
        title: "Reduce Intensity",
        message:
          "Moderate fatigue detected. Consider reducing activity intensity by 30-50%.",
        action: "Reduce Load",
      });
    } else if (fatigue === "Slight Fatigue") {
      suggestions.push({
        ...
        title: "Monitor Activity",
        message:
          "Slight fatigue building up. Take periodic breaks every 10-15 minutes.",
        action: "Schedule Breaks",
      });
    }
    ...
    if (activity === "Intense") {
      suggestions.push({
        ...
        title: "High Intensity Detected",
        message:
          "Maintain proper form and breathing technique. Stay hydrated.",
        action: "Check Form",
      });
    }
    ...
    if (mag === 0 && raw === 0) {
      suggestions.push({
        ...
        title: "Check Sensor Connection",
        message:
          "No EMG signal detected. Verify electrode placement and skin contact.",
        action: "Check Electrodes",
      });
    }
    ...
    if (!isNaN(temp)) {
      if (temp > 38) {
        suggestions.push({
          ...
          title: "Elevated Temperature",
          message:
            "Body temperature is elevated. Cool down, hydrate, and rest. Seek medical attention if persistent.",
          action: "Cool Down",
        });
      } else if (temp < 36) {
        suggestions.push({
          ...
          title: "Low Temperature",
          message:
            "Body temperature is low. Warm up gradually and monitor for symptoms.",
          action: "Warm Up",
        });
      }
    }
    ...
    if (suggestions.length === 0) {
      suggestions.push({
        ...
        title: "All Systems Normal",
        message:
          "Continue current activity. Maintain good posture and stay hydrated.",
        action: "Continue",
      });
      ...
      suggestions.push({
        ...
        title: "Stay Hydrated",
        message:
          "Remember to drink water regularly, especially during physical activity.",
        action: "Drink Water",
      });
    }
  ...
  const startDemoFeed = (type) => {
    ...
    demoIntervalRef.current = setInterval(() => {
      ...
      if (type === "normal") {
        raw = 20 + Math.random() * 55;
        mag = 18 + Math.random() * 45;
        temp = 36.2 + Math.random() * 0.9;
        hum = 40 + Math.random() * 18;
      }

      if (type === "paralysis") {
        raw = Math.random() * 5;
        mag = Math.random() * 6;
        temp = 35.5 + Math.random() * 0.7;
        hum = 45 + Math.random() * 20;
      }

      const demoLine = `EMG raw=${raw.toFixed(2)} mag=${mag.toFixed(
        2
      )} thr=${thr.toFixed(1)} Temp:${temp.toFixed(1)} Hum:${hum.toFixed(1)}`;

      processSerialData(demoLine);
    }, 450);
  };
  ...
  useEffect(() => {
    ...
    ctx.strokeStyle = demoMode === "paralysis" ? "#fc8181" : "#48bb78";
    ctx.lineWidth = 2.5;
    ctx.beginPath();
    ...
    <div className="emg-serial-container">
        ...
        <div className="connection-info">
          ...
          <span
            className={`status-indicator ${
              isConnected || demoMode ? "connected" : "disconnected"
            }`}
          >
            {isConnected
              ? "● Connected"
        ...
        <div className="waveform-section">
          ...
          <div className="waveform-container">
            <div className="waveform-header">
              <div className="waveform-info">
                <span className="waveform-label">Current EMG Value:</span>
                <span className="waveform-value">
                  {emgValue !== null ? emgValue.toFixed(2) : "--"}
                </span>
              </div>
              ...
              <div className="waveform-status">
                <span className="status-dot"></span>
                <span>{demoMode ? `${demoMode} demo` : "Real-time"}</span>
              </div>
            </div>
            ...
            {emgData.length > 0 ? (
              ...
              <div className="waveform-placeholder">
                ...
                <p className="help-text">
                  EMG waveform will appear here once data is received
                </p>
              </div>
            )}
          </div>
      ...
      <div className="prediction-section">
        ...
        <div className="prediction-grid">
          <div className="prediction-card activity-prediction">
            ...
            <div className="prediction-value">{prediction.activity}</div>
          ...
          <div className="prediction-card gesture-prediction">
            ...
            <div className="prediction-value">{prediction.gesture}</div>

            <div className="prediction-label">Detected Pattern</div>
          </div>
          ...
          <div className="prediction-card fatigue-prediction">
            ...
            <div className="prediction-value">{prediction.fatigue}</div>

            <div className="prediction-label">Based on 20-sample average</div>
          </div>
        </div>
      </div>
      ...
      <div className="dashboard-grid">
        ...
        <div className="card data-count-card">
          ...
          <div className="count-display">{serialData.length}</div>
          <div className="count-label">Total Received</div>
        </div>
        ...
        <div className="card latest-raw-card">
          ...
          <div className="timestamp">
            {serialData.length > 0
              ? serialData[serialData.length - 1].time
              : "--:--:--"}
          </div>
        </div>
      </div>
          ...
          <table className="data-table">
            ...
            <tbody>
              ...
              {serialData.length === 0 && (
                <tr>
                  <td colSpan="4" className="no-data">
                    No data received yet. Connect to serial port to start
                    monitoring.
                  </td>
                </tr>
              )}
      ...
      {/* ✅ SMALL HIDDEN DEMO BUTTONS AT LAST */}
      <div className="demo-hidden-panel">
        {/* <span className="demo-hidden-title">Demo Feed</span> */}

        <button
          className={`demo-mini-btn normal ${
            demoMode === "normal" ? "active" : ""
          }`}
          onClick={() => startDemoFeed("normal")}
        >
          
        </button>

        <button
          className={`demo-mini-btn paralysis ${
            demoMode === "paralysis" ? "active" : ""
          }`}
          onClick={() => startDemoFeed("paralysis")}
        >
          
        </button>

        <button className="demo-mini-btn stop" onClick={stopDemoFeed}>
          -
        </button>
      </div>
    </div>
